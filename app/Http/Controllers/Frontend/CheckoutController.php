...
class CheckoutController extends Controller
{
    public function index()
    {
        ...
        if (Auth::user()->no_hp == null) {
            return redirect()->route('settingprofile')->with('error', 'Silahkan lengkapi data diri anda terlebih dahulu');
        } else {
            if ($cart_check > 0) {
                return view('frontend.order.index', compact('produk', 'payment'));
            } else {
